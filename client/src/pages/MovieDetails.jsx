...
const getShow = useCallback(async ()=>{
    try {
        // Fix: correct endpoint path to match server route '/api/shows/:movieId'
...
const handleWatchTrailer = () => {
    // navigate to home then scroll to the TrailerSection anchor
    navigate('/');
    // small timeout to allow route change / DOM to update
    setTimeout(() => {
            ...
            <p>
             {timeFormat(show.movie.runtime)} • {show.movie.genres.map(genre => genre.name).join(", ")} • {show.movie.release_date.split("-")[0]}
                     ...
                     <div className='flex item-center flex wrap gap-4 mt-4'>
                         <button onClick={handleWatchTrailer} className='flex items-center gap-2 px-7 py-3 text-sm bg-gray-800 hover:bg-gray-900 transition rounded-md font-medium cursor-pointer active:scale-95'>
